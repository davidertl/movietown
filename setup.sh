...
create_env_file() {
    ...
    if [ "$DEPLOY_MODE" = "cloud" ]; then
        ...
        sed -i "s|AUTHENTIK_REDIS__HOST=.*|AUTHENTIK_REDIS__HOST=${REDIS_HOST}|g" "$ENV_FILE"
        sed -i "s|AUTHENTIK_POSTGRESQL__HOST=.*|AUTHENTIK_POSTGRESQL__HOST=${POSTGRES_HOST}|g" "$ENV_FILE"
